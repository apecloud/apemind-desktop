...
本文延续 [ApeMind CLI-first 集成设计](2026-09-14-apemind-cli-first-integration.md) 的单一客户端、凭据和工作空间边界，并扩展通用 API、MCP、业务命令和能力发现方案。能力差距分析保留 2026-09-16 的调查快照，不作为今天的完成清单；建议命令和事件示例属于目标设计，不代表已实现。具体参数和输出合同需要在实现对应能力时定稿。当前系统没有统一的 `task` 资源，`task` 也不是 CLI 的用户可见或内部持久化概念；等待、查看进度和取消只附着在服务端已经定义的资源或事件上。
...
7. 当前系统没有统一 `task` 资源，`task` 不是 ApeMind 的产品概念。CLI 不创建公共的 `task list/get/cancel` 抽象，不把一次命令执行记录保存成任务，也不在内部用 `task` 作为跨资源状态名。等待、查看进度和取消必须使用服务端已经定义的文档、导入、导出、Turn 或其他资源状态和事件；CLI 只提供对应资源的轮询、退避和中断处理。没有取消接口就不提供取消命令，停止等待不代表取消服务端执行。
