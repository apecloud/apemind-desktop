...
用户在 Desktop 中点击一次“登录 ApeMind”，即可完成账户登录，并看到当前身份实际可访问的工作空间。列表可能包含历史遗留个人空间、一个或多个组织空间，也可能为空。Desktop 保存的是可撤销的 OAuth 设备会话，不创建隐藏 API Key，也不把浏览器 Cookie 当作桌面长期登录态。每次业务请求仍由服务端根据用户身份、组织成员关系、角色和 scope 做最终鉴权。
...
服务端只接受当前用户实际可见且状态为 active 的工作空间。组织空间是长期默认边界，遗留个人空间只有在用户状态允许且资源存在时才可用；工作空间列表可以为空。组织切换不会改变用户身份，也不会绕过组织 RBAC。当前 OAuth scope 只允许账户、工作空间和知识库读取，修改型操作必须增加独立 scope 和业务审计。
