...
授权确认页继续校验同源来源。优先使用 `Origin`，缺少时解析同站 `Referer`；两者都缺失或不在可信公网来源集合中时拒绝确认。这个校验只负责网页表单的 CSRF 边界，Desktop 主流程的安全性由 PKCE、回环监听、设备码和一次性状态保证。
...
- 浏览器缺少 `Origin` 时可使用同站 `Referer`，两个来源头都缺失时拒绝网页表单并提示改用设备码。
