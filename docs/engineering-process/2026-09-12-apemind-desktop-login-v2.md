...
用户在 Desktop 中点击一次“登录 ApeMind”，即可完成账户登录，并看到当前身份实际可访问的工作空间。列表可能包含历史遗留个人空间、一个或多个组织空间，也可能为空。线上新账户默认没有个人空间；历史账户可能保留个人空间、保留一个空个人空间，或已经迁移到组织而完全没有个人空间。Desktop 保存的是可撤销的 OAuth 设备会话，不创建隐藏 API Key，也不把浏览器 Cookie 当作桌面长期登录态。每次业务请求仍由服务端根据用户身份、组织成员关系、角色和 scope 做最终鉴权。
...
登录成功后，页面显示用户名、服务地址、工作空间选择器和最近同步时间。工作空间列表以服务端返回为准，显示类型、组织角色和可用权限。重新登录时保留仍然 active 的历史选择；没有有效历史选择且只有一个组织空间时自动选择该组织；只有一个遗留个人空间时为兼容旧账户选择它；多个组织或空列表都要求用户明确处理。暂停、撤销或不可访问的空间不能选择。点击切换后，Desktop 重新向服务端获取列表并校验目标空间仍然有效；失败时保留原选择。

登录成功不等于一定拥有工作空间。列表为空时页面说明账号已经登录、当前没有可用空间，并提供刷新、加入组织或打开帮助的入口；知识库、模型、Agent、Chat 和 Turn 等需要空间的入口显示明确的“先选择工作空间”状态，不发送猜测请求。
...
服务端只接受 `/api/v2/me/workspaces` 返回且当前用户实际可见的工作空间。组织空间是长期默认边界，遗留个人空间只有在资格和迁移状态允许时才可用，个人空间可以为空；工作空间列表可以为空。组织切换不会改变用户身份，也不会绕过组织 RBAC。当前 OAuth scope 只允许账户、工作空间和知识库读取，修改型操作必须增加独立 scope 和业务审计。
...
5. 登录后显示服务端返回的全部有效工作空间；个人空间缺失或工作空间列表为空时仍能完成登录并显示可操作提示；
