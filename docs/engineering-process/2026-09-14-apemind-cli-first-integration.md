...
每次服务端权限变化都要同时验证 OAuth 和 API Key 两条路径。API Key 创建 Chat 或 Turn 时记录当时的连接身份和权限快照；后续请求必须继续使用同一连接，且当前空间与资源权限覆盖快照。知识库转移、组织成员变化、Key 收窄或撤销后，旧历史、摘要、证据、附件和导出都必须按同一边界拒绝。这个验证闭环属于 CLI 与服务端交付证据，Desktop 不复制一份。
