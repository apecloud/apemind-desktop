...
CLI 不自行创造服务端不存在的公共资源。当前系统没有统一的 `task` 资源，CLI 不新增 `task` 命令或本地任务状态机。索引、导入、导出和 Turn 等长操作沿用服务端已有的资源状态或事件；CLI 可以据此提供对应资源的等待或事件查看，只有服务端支持取消时才提供取消命令，一次命令执行本身不变成可持久化的公共资源。后续交付以 CLI 为主线，Desktop 只跟随登录、打包、调用与结果展示的必要集成变化。
