...
后续命令体验、通用 API、MCP 和业务能力扩展见 [ApeMind CLI 对标 gh 的能力分析与产品技术设计](2026-09-16-apemind-cli-gh-capability-design.md)。个人空间的可选化、空工作空间和组织长期迁移见[个人空间可选化与组织工作空间迁移设计](2026-09-26-personal-workspace-transition.md)。本文继续定义统一身份、凭据与进程调用边界；普通用户、平台 admin、workspace/organization 语义和授权矩阵见 [ApeMind CLI 命令语义、权限范围与管理能力设计修订](2026-09-24-apemind-cli-command-scope-and-admin-design.md)，通用 API 的开放范围以相关设计为准。
...
Desktop 的 UI、设置项和后台流程也调用同一个 CLI。用户在 UI 中看到的状态，应当与执行 `apemind auth status --format json` 得到的状态一致。
...
CLI 不自行创造服务端不存在的公共资源。当前系统没有统一的 `task` 资源，`task` 也不是 ApeMind 的产品概念；CLI 的命令、帮助、结构化输出和本地状态都不使用它做跨资源的统一抽象。索引、导入、导出和 Turn 等长操作沿用服务端已有的资源状态或事件；CLI 可以据此提供对应资源的等待或事件查看，只有服务端支持取消时才提供取消命令，一次命令执行本身不变成可持久化的公共资源。服务端内部出现 `job`、队列记录或类似名称时，也只有在它作为公开资源提供并具有独立权限边界时，才映射成对应领域命令。后续交付以 CLI 为主线，Desktop 只跟随登录、打包、调用与结果展示的必要集成变化。
...
### Chat 与 API Key 的历史边界

API Key 连接不能因为用户账号和工作空间相同，就读取另一条 API Key 连接产生的 Chat、Turn、摘要、附件或证据。服务端在创建 Chat 时记录连接的 API Key 身份和当时的权限快照；后续 API Key 请求必须使用同一个连接，并且当前权限覆盖创建时快照。权限扩大可以继续使用，权限缩小、Key 更换、Key 已撤销或无法证明旧历史来源时，服务端拒绝读取并提示创建新的 Chat。OAuth 连接按用户当前的工作空间和资源权限读取，不从浏览器 Cookie 或其他连接借用身份。

该边界覆盖 Chat 列表、首个 Chat、侧栏、详情、Turn 目录、Turn 快照、证据、附件、导出、摘要和下一轮模型上下文。新 Turn 在配额扣减、附件处理和模型调用前重新检查边界；旧历史没有可靠权限来源时整体拒绝，不尝试只删引用或改写自由文本。这个设计沿用已有 Chat、Turn 和工作空间权限，不创建统一的客户端 `task` 资源。

实现上，服务端在 Chat 行维护 API Key 连接身份、创建时权限快照和所有已验证 Turn 权限的单调并集；新 Turn 在同一事务内锁定 Chat、重新验证调用方身份并合并快照，然后才进入配额、附件和模型路径。列表、总数、侧栏、详情、目录、导出和历史读取先用数据库边界筛选，再做必要的 JSON scope 校验；delegation 的子 Turn 读取、自动 salvage 和引用 materialization 也复用同一条 Chat 历史边界。这样权限判断不会随 Turn 数量线性加载历史，也不会因为 Desktop 绕过 CLI 而出现第二套规则。

### 资源状态是唯一的异步事实来源

CLI 的一次执行只是一个前台进程。它可以发起请求、等待响应、轮询资源状态或订阅服务端事件，但不会把这次执行登记成一个新的公共对象。CLI 不生成跨资源的执行 ID，不保存本地任务表，也不提供 `task list`、`task get`、`task wait` 或 `task cancel`。

这里的边界是产品定义，不只是命令命名偏好：ApeMind 当前没有统一的 `task` 资源，CLI 也不应把“任务”作为自己的领域对象。命令、输出字段、事件类型、缓存键和本地状态都使用服务端已经存在的资源名称。中文说明中可以描述“等待一次操作”，但这句话不代表存在一个可以被查询、授权、分享或取消的 Task 对象。

需要等待时，命令必须明确它等待的服务端资源、状态字段和结束条件：

| 用户意图 | CLI 允许的形态 | 状态来源 |
|---|---|---|
| 等待文档完成处理 | `document wait <document-id>` | 文档自身的处理状态或服务端事件 |
| 等待一次 Turn | `turn wait <turn-id>` 或 `turn watch <turn-id>` | Turn 状态和事件流 |
| 观察导入、导出或同步 | 对应领域的 `import`、`export` 或 `sync` 命令 | 服务端公开的对应资源；没有资源就直接返回请求结果 |
| 停止等待 | 终止本地等待或断开事件流 | 不改变服务端执行 |
| 取消执行 | 只在对应服务端资源提供取消接口时提供 | 服务端资源的取消状态 |

轮询、退避、超时、断线重连和终端中断属于 CLI 的实现细节，不构成可被其他命令读取的状态。服务端内部的队列记录、作业对象或数据库字段只有在通过公开接口提供独立身份、权限和生命周期时，才映射为对应领域命令。没有公开资源时，CLI 不为它再包一层本地模型。

### 命令合同中的禁止项

“不引入统一 `task`”必须落实到机器合同，不能只停留在命令名称上。CLI 和 Desktop 的结构化输出、事件流、缓存和错误对象都不能使用一个跨资源的 `task_id`、`task_status` 或 `task_type` 来包住不同领域的操作。每个字段都应回答它属于哪个真实资源，例如 `document_id`、`turn_id`、`export_id` 或服务端公开的导入资源 ID。

一次命令执行可以有本地请求上下文、超时和重试次数，但这些信息只描述本次进程调用，不得被保存为下一次命令可以查询的业务状态。Desktop 可以把 CLI 输出显示为“正在等待文档处理”或“正在等待 Turn 完成”，但界面状态必须能追溯到对应资源的服务端状态或事件；关闭窗口、终止进程和断开网络只结束本地等待，不改变服务端资源。

实现新能力前，必须先回答三个问题：

1. 服务端是否已经公开了可被用户识别、授权和查询的真实资源？
2. 该资源是否有明确的状态、生命周期和权限边界？
3. CLI 是否能直接用资源名称表达命令、输出和错误？

三个问题中任意一个答案为“否”，就不新增跨资源状态对象；先补齐真实资源合同，或者把操作设计成一次性请求。
...
`/me/workspaces` 返回当前身份可访问空间的统一投影。个人空间只有在服务端启用且资源存在时才返回，列表可以为空。组织成员资格、角色、状态和能力由服务端决定，客户端只保存显示快照。
...
当前交付主线只推进 ApeMind CLI 及其服务端合同。Desktop 暂时只承担登录入口、内置 CLI 的分发和版本回归；除非 CLI 合同无法落地，不新增 Desktop 专用业务逻辑。

Turn 相关能力已经沿用服务端资源模型推进：`turn create`、`turn cancel`、`turn watch`、`turn consent`、`turn elicit` 和 `turn wait` 都直接作用于已有的 Chat/Turn/事件资源。`turn wait` 只是 CLI 在前台轮询 Turn 状态的行为，超时或终止等待不会创建、保存或取消任何额外对象。API Key 不支持事件流时，使用同一个 Turn 状态接口轮询；OAuth 可以在权限允许时观看服务端事件。

后续每增加一个异步命令，都必须先回答三个问题：服务端公开的资源是什么、成功和失败状态由哪个字段或事件定义、服务端是否提供取消接口。回答不了时，先补服务端资源合同或直接保持同步调用，不在 CLI 侧发明统一任务、作业或执行记录。
...
代码评审和命令设计使用下面的判据：如果提案不能指出服务端公开的资源、状态、权限和生命周期，就不能新增对应的异步 CLI 命令；如果只能通过生成客户端 ID、临时文件、隐藏线程或本地数据库来补齐这些信息，应先补服务端合同或保持同步调用。服务端未来若公开统一资源，CLI 直接映射该资源的字段和权限，不再套一层同名或近似的客户端抽象。
...
每次服务端权限变化都要同时验证 OAuth 和 API Key 两条路径。API Key 创建 Chat、Turn 或 Turn 导出时记录当时的连接身份和权限快照；后续请求必须继续使用同一连接，且当前空间与资源权限覆盖快照。知识库转移、组织成员变化、Key 收窄或撤销后，旧历史、摘要、证据、附件和导出都必须按同一边界拒绝。这个验证闭环属于 CLI 与服务端交付证据，Desktop 不复制一份。
